...
function frameworkSystemPrompt(opts?: {
  ...
  if (opts?.capability) {
    lines.push(
      ...
      "写代码：短脚本放沙箱；要常驻能力就写到 plugins/ 再 reload。查文件、跑系统命令、动服务器用 nexus_shell，工作目录默认框架根，也可指定绝对路径。",
      ...
      "对用户说人话。思考过程可以写，框架会用合并转发发出；正文按空行拆成最多两条气泡。不要把「用某某语气」「不需要调用工具」这类提纲写进正文。不要把工具名、JSON、DSML 甩出去。",
...
async function bootstrap(): Promise<void> {
  ...
  async function processInbound(
    ...
    if (!all.length && !thinkingForwarded) return [];
    if (!all.length && thinkingForwarded) return [];
